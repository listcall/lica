...
def mem_label(member)
  ...
  ol_label = "" #member.ol ? "OL " : ""
  str_truncate "#{ol_label}#{member.rank} #{member.full_name}", 26
end

def phone_for(member)
  member.phones.mobile.visible_for(current_membership).first.try(:number) || member.phones.work.visible_for(current_membership).first.try(:number)
end

def email_for(member)
  adr = member.emails.personal.visible_for(current_membership).first.try(:address) ||
  member.emails.work.visible_for(current_membership).first.try(:address)     ||
  member.emails.home.visible_for(current_membership).first.try(:address)     ||
  member.emails.other.visible_for(current_membership).first.try(:address)
  str_truncate adr, 28
end
...
prawn_document(:border_width => 0, :page_layout => :landscape, :left_margin => 22, :top_margin => 18, :bottom_margin => 18) do |pdf|
  ...
  #pdf.text "Updated #{Time.now.strftime("%Y-%m-%d %H:%M")} / #{team_name} Confidential"

  pdf.font_size 12

  pdf.draw_text "#{team_name} Wallet Roster",       :at=>[475, 550]
  ...
  pdf.canvas do
    pdf.draw_text "#{team_name} Roster",                                   :at=>[20, 510], :rotate => 90, :size => 8
    pdf.draw_text "#{team_name} Confidential - #{Time.now.strftime("%Y-%m-%d")}", :at=>[20, 175], :rotate => 90, :size => 6
    pdf.line_width=0.25
