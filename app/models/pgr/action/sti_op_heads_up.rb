...
class Pgr::Action::StiOpHeadsUp < Pgr::Action
  ...
  class << self
    ...
    def about
      <<-EOF.gsub('      ','').gsub("\n", ' ')
        Heads Up RSVP allows the page recipient to reply with a
    ...
    def usage_ctxt
      'Requesting availability for event'
    end

    def short_msg
      "HEADS UP: {event_title}/OP{period_num} Would you be available?"
    end
