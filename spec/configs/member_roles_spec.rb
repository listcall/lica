...
describe MemberRoles do
  ...
  context 'basic object creation' do
    ...
    before(:each) { @obj = MemberRoles.new }
    ...
    context 'with hash params' do
      ...
      it 'works with one hash' do
        @obj = MemberRoles.new.add_data({a: valid_params})
        expect(@obj).not_to be_nil
      end
    end
  ...
  describe '.[]' do
    before(:each) do
      @obj1 = FG.build :member_role
      @obj2 = FG.build :member_role
      @col  = MemberRoles.new.add_obj @obj1, @obj2
    end
  ...
  describe '.count' do
    ...
    before(:each) { @col = MemberRoles.new }
    ...
    context 'with two objects' do
      it 'returns count of two' do
        @col.set_obj(new_member_role, new_member_role)
    ...
    context 'after destroying an object' do
      it 'returns count of one' do
        mdl1 = new_member_role
        mdl2 = new_member_role
    ...
    context 'with two objects' do
      context 'when a key hits' do
        it 'returns an object' do
          mdl1 = new_member_role
          @col.set_obj mdl1, new_member_role
  ...
  describe '#to_a' do
    context 'with no objects' do
      specify {expect(MemberRoles.new.to_a).to be_a Array }
      specify {expect(MemberRoles.new.to_a.count).to eq(0) }
    end

    context 'with two objects' do
      before(:each) { @col = MemberRoles.new.set_obj new_member_role, new_member_role}
      it 'generates a valid array' do
        expect(@col.to_a).to be_an Array
        expect(@col.to_a.first).to be_a MemberRole
  ...
  describe '#to_h' do
    context 'with no objects' do
      specify {expect(MemberRoles.new.to_h).to be_a Hash }
      specify {expect(MemberRoles.new.to_h.count).to eq(0)}
    end
    ...
    context 'with two objects' do
      ...
      it 'generates a valid hash' do
        ...
        expect(result[result.keys.first]).to be_a MemberRole
  ...
  describe '#to_data' do
    context 'with no objects' do
      specify {expect(MemberRoles.new.to_data).to be_a Hash }
      specify {expect(MemberRoles.new.to_data.count).to eq(0)}
    end
