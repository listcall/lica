...
class Pgr::Action::StiOpImmediateCallout < Pgr::Action
  ...
  class << self
    ...
    def about
      <<-EOF.gsub('      ','').gsub("\n", ' ')
        Immediate Callout RSVP allows the page recipient to reply with a
    ...
    def usage_ctxt
      'Requesting immediate attendance for event'
    end

    def short_msg
      "IMMEDIATE CALLOUT: {event_title}/OP{period_num} Are you available? "
    end
