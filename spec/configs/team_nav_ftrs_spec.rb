...
describe TeamNavFtrs do
  ...
  context 'basic object creation' do
    ...
    before(:each) { @obj = TeamNavFtrs.new }
    ...
    context 'with hash params' do
      ...
      it 'works with one hash' do
        @obj = TeamNavFtrs.new.add_data({a: valid_params})
        expect(@obj).not_to be_nil
      end
    end
  ...
  describe '.[]' do
    before(:each) do
      ...
      @col  = TeamNavFtrs.new.add_obj @obj1, @obj2
    end
  ...
  describe '.count' do
    ...
    before(:each) { @col = TeamNavFtrs.new }
  ...
  describe '#to_a' do
    context 'with no objects' do
      specify {expect(TeamNavFtrs.new.to_a).to be_a Array }
      specify {expect(TeamNavFtrs.new.to_a.count).to eq(0) }
    end

    context 'with two objects' do
      before(:each) { @col = TeamNavFtrs.new.set_obj new_team_menu, new_team_menu}
  ...
  describe '#to_h' do
    context 'with no objects' do
      specify {expect(TeamNavFtrs.new.to_h).to be_a Hash }
      specify {expect(TeamNavFtrs.new.to_h.count).to eq(0)}
    end
  ...
  describe '#to_data' do
    context 'with no objects' do
      specify {expect(TeamNavFtrs.new.to_data).to be_a Hash }
      specify {expect(TeamNavFtrs.new.to_data.count).to eq(0)}
    end
