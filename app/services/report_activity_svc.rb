...
class ReportActivitySvc
  ...
  def periods_for(events)
    ...
    Event::Period.where(event_id: event_ids)
  end

  def participants_for(periods)
    period_ids = periods.pluck(:id)
    Event::Participant.where(event_period_id: period_ids)
  end
  ...
  def trn_p; @tp ||= Event::Period.where(event_id: trn_lst.pluck(:id)); end
  def opr_p; @op ||= Event::Period.where(event_id: opr_lst.pluck(:id)); end
  def mtg_p; @mp ||= Event::Period.where(event_id: mtg_lst.pluck(:id)); end
  def com_p; @cp ||= Event::Period.where(event_id: com_lst.pluck(:id)); end

  def par_for(per, mem)
    Event::Participant.where(event_period_id: per.pluck(:id), membership_id: mem.id)
  end
  ...
  def members
    memdata = team.memberships.active.all.map do |mem|
      {
        id:   mem.id,
        name: mem.full_name,
        lname: mem.last_name,
