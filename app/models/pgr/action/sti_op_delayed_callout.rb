...
class Pgr::Action::StiOpDelayedCallout < Pgr::Action
  ...
  class << self
    ...
    def about
      <<-EOF.gsub('      ','').gsub("\n", ' ')
        Delayed Callout RSVP allows the page recipient to reply with a
    ...
    def usage_ctxt
      'Requesting delayed attendance for event'
    end

    def short_msg
      "DELAYED CALLOUT: {event_title}/OP{period_num} Are you available? "
    end
