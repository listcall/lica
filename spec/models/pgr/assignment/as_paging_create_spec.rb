...
describe Pgr::Assignment::AsPagingCreate do
  ...
  describe '#sequence_id' do
    let(:asn) { pgr.assignments.create.becomes(klas) }
    let(:pgr) { Pgr.create                           }
  ...
  describe '#broadcast' do
    it 'generates a live broadcast from the assignment' do
      subject.save
      expect(subject.broadcast).to be_nil
      subject.create_broadcast
      expect(subject.broadcast).to_not be_nil
      expect(subject.broadcast).to be_a(Pgr::Broadcast::AsPagingCreate)
    end
  end
end
